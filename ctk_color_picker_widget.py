...
class CTkColorPicker(customtkinter.CTkFrame):
    ...
    def __init__(self,
        ...
        self.slider = customtkinter.CTkSlider(master=self, width=20, border_width=self.slider_border,
                                              button_length=1, progress_color=self.default_hex_color, from_=0, to=255,
    ...
    def update_colors(self, er=None, eg=None, eb=None):
        brightness = self.brightness_slider_value.get()
        if (er==None and eg==None and eb==None and self.rgb_avail==False):
            self.get_target_color() 
        else:
            self.rgb_avail = True
